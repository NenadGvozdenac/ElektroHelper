...
public class GetPostsByFollowersPagedHandler(IGraphDatabaseContext context) : IRequestHandler<GetPostsByFollowersPagedQuery, Result<List<PostDTO>>>
{
    public async Task<Result<List<PostDTO>>> Handle(GetPostsByFollowersPagedQuery request, CancellationToken cancellationToken)
    {
        var query = @"
