...
public class GetPostsByForumIdPagedHandler(IGraphDatabaseContext context) : IRequestHandler<GetPostsByForumIdPagedQuery, Result<List<PostDTO>>>
{
    public async Task<Result<List<PostDTO>>> Handle(GetPostsByForumIdPagedQuery request, CancellationToken cancellationToken)
    {
        var query = @"
