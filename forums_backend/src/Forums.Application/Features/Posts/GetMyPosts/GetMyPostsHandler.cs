...
public class GetMyPostsHandler(IGraphDatabaseContext context) : IRequestHandler<GetMyPostsQuery, Result<List<PostDTO>>>
{
    public async Task<Result<List<PostDTO>>> Handle(GetMyPostsQuery request, CancellationToken cancellationToken)
    {
        var query = @"
